...
/**
 * Shared Playwright helpers for EduAI UI demo recordings.
 * Used by demo-full-feature.cjs (rehearse + record).
 */
  ...
  if (!visible) {
    ...
    const found = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('button, input, select, textarea, a'))
        ...
        .map(
          (node) =>
            `${node.tagName}[${node.type || ''}] "${(node.textContent || '').trim().substring(0, 30)}"`,
        )
        .join('\n  ');
    });
...
async function moveAndClick(page, locator, label, opts = {}) {
  const { postClickDelay = 800, timeout = 8000, ...clickOpts } = opts;
  const el = typeof locator === 'string' ? page.locator(locator).first() : locator;
  const visible = await el.isVisible().catch(() => false);
  if (!visible) {
  ...
  try {
    ...
    await el.click({ timeout, ...clickOpts });
  } catch (e) {
...
async function gotoWithOverlays(page, url) {
